...
const PaymentGateway = () => {
  ...
  const getNGONames = async () => {
    try {
      const response = await fetch("http://localhost:8080/api/getNGO").then(
        (res) => res.json()
      );
      if (response.status === "ok") {
        ...
        if (response.data) {
          ...
          alert("Transactions are not available as of now, please try again later");
          setTimeout(() => navigate("../", { replace: true }), 10000);
        }
      } else {
        alert("Failed to get NGO data");
      }
    } catch (error) {
      alert("Transactions are not available as of now, please try again later");
    }
  ...
  const handleSubmit = async (event) => {
    ...
    if (!selectedNGO) {
      alert("NGO Name must be selected");
      return;
    }
    ...
    console.log("Selected Country:", selectedNGO);
            ...
            <div className="recipt-container">
              <div className="flex flex-col justify-between p-4 align-center credit-card border">
                ...
                <div>
                  ...
                  <p className="card-no card-text">
                    <span className="hidden-no">
                      {cardNumber
                        ? cardNumber.slice(0, cardNumber.length - 5)
                        : "123456789789"}
                    </span>
                    {cardNumber
                      ? cardNumber.slice(
              ...
              <div className="checkout-details">
                ...
                >
                  Mode:{" "}
                  <span style={{ fontWeight: "600" }}>{gatewayData.mode}</span>
                </h2>
                <hr
                  className="checkout-hr"
                  style={{ width: "90%", margin: "0 auto" }}
                />
              </div>
              <div
                className="ammount-c"
                style={{ width: "90%", margin: "0 auto", color: "#1f2b53" }}
              >
                <p className="text-center">You have to Pay</p>
                <h1
                  className="d-ammount text-center"
                  style={{
                    position: "relative",
                    bottom: "20px",
                    fontWeight: "600",
...
const Options = ({ NGOs }) => {
  if (!NGOs) return <option value="">NO NGOs are available</option>;
  return NGOs.map((item, id) => {
    return (
      <option className="ngo-option" key={id} value={item}>
        {item}
      </option>
    );
  });
